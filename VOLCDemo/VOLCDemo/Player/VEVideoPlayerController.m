...
- (void)videoEngineWillPrepare:(TTVideoEngine *)videoEngine {
    [VEVideoPlayerController setVideoEngineOptions:videoEngine];
}
...
- (void)configVideoEngine {
    ...
    [VEVideoPlayerController setVideoEngineOptions:self.videoEngine];
}
...
- (void)__addPeriodicTimeObserver {
    ...
    [self.videoEngine addPeriodicTimeObserverForInterval:0.3f queue:dispatch_get_main_queue() usingBlock:^{
        @strongify(self);
        if ([self.delegate respondsToSelector:@selector(videoPlayerViewSizeDidChange:timeIntervalDidChanged:)]) {
            [self.delegate videoPlayerViewSizeDidChange:self timeIntervalDidChanged:self.currentPlaybackTime];
        }
        if ([self.receiver respondsToSelector:@selector(playerCore:playTimeDidChanged:info:)]) {
            [self.receiver playerCore:self playTimeDidChanged:self.currentPlaybackTime info:@{}];
        }
    }];
}
...
- (void)videoEnginePrepared:(TTVideoEngine *)videoEngine {
    if ([self.delegate respondsToSelector:@selector(videoPlayerPrepared:)]) {
...
- (void)videoEngineReadyToDisPlay:(TTVideoEngine *)videoEngine {
    ...
    if ([self.delegate respondsToSelector:@selector(videoPlayerReadyToDisplay:)]) {
...
- (void)videoEngine:(TTVideoEngine *)videoEngine mdlKey:(NSString *)key hitCacheSze:(NSInteger)cacheSize {
    if ([self.delegate respondsToSelector:@selector(videoPlayer:key:hitVideoPreloadDataSize:)]) {
...
- (void)videoBitrateDidChange:(TTVideoEngine *)videoEngine resolution:(TTVideoEngineResolutionType)resolution bitrate:(NSInteger)bitrate {
    if ([self.delegate respondsToSelector:@selector(videoPlayerBitrateDidChange:resolution:bitrate:)]) {
...
- (void)videoSizeDidChange:(TTVideoEngine *)videoEngine videoWidth:(NSInteger)videoWidth videoHeight:(NSInteger)videoHeight {
    if ([self.delegate respondsToSelector:@selector(videoPlayerViewSizeDidChange:videoWidth:videoHeight:)]) {
...
- (void)__handlePlaybackStateChanged:(VEVideoPlaybackState)state {
    ...
    if ([self.delegate respondsToSelector:@selector(videoPlayer:playbackStateDidChange:)]) {
...
- (void)__handleLoadStateChanged:(VEVideoLoadState)state {
    ...
    if ([self.delegate respondsToSelector:@selector(videoPlayer:loadStateDidChange:)]) {
